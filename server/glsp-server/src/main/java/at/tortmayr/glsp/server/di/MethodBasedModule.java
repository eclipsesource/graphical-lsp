...
public abstract class MethodBasedModule implements Module {
	private static Logger LOGGER = Logger.getLogger(MethodBasedModule.class);
		...
		if (isClassBinding()) {
			...
			if (value != null && !Void.class.equals(value)) {
				if (LOGGER.isTraceEnabled()) {
					LOGGER.trace("Adding binding from " + key + " to " + value.getName() + ". Declaring Method was '"
							+ getMethod().toGenericString() + "' in Module " + this.getClass().getName());
				}
			...
			if (LOGGER.isTraceEnabled())
				LOGGER.trace("Adding binding from " + getMethod().getReturnType().getName() + " to instance "
						+ instance.toString() + ". Declaring Method was '" + getMethod().toGenericString()
	...
	protected IllegalStateException throwIllegalReturnTypeDeclaration(Method method) {
		return new IllegalStateException("return type of " + method.getName()
				+ " should be declared with wildcard and upperbound (i.e. Class<? extends IScopeProvider>)");
	}
